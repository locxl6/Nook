...
export default function ModelSelectorHeader() {
  ...
  const handleDownload = async (modelName: string) => {
    ...
    try {
      const res = await fetch(
        `${settings.baseUrl}/api/model/pull?model_name=${encodeURIComponent(modelName)}`,
  ...
  const options = [
    ...
    { type: 'divider' as const },
    {
      value: '__download__',
      label: (
        <span style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--ds-accent)' }}>
          <DownloadOutlined />
          <span>下载模型</span>
        </span>
      )
  ...
  const isInstalled = (name: string) =>
    models.some((m) => m.name === name || m.id === name) || downloadedModels.has(name)
